...
Set mediaEdit true when the user asks to EDIT an existing clip instead of generating new media: grab a frame/screenshot of a video, split/trim/cut a segment, join/concatenate clips, crop/resize/rotate a clip (e.g. to a 16:9 shape or to fit a size limit), change a clip's playback speed (speed it up, slow motion), extract the audio track, or put different audio under a video. The responseMode for these stays "text" — the edited clip is attached to the reply, not generated. When one of the edit tools (screenshot_video, split_video, join_videos, transform_video, extract_audio, replace_audio) is listed under Available tools, set needsTools true and describe the edit in toolTask; when none is listed, set needsTools false — the harness itself tells the user how to enable local editing.
