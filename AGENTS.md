...
| `openai_compatible_client.go` | HTTP client for a local server speaking OpenAI's `/v1/images/generations` shape (LocalAI, a diffusers shim, ...) — the third image provider (`"openai-compatible"`), an image-only backend like fal. Packs results into `ollamaGenerateResponse` data URLs with nil raw JSON; shares `fetchImageAsDataURL` with the fal client. |
