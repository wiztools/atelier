...
const defaultFalVideoRestyleModel = 'fal-ai/kling-video/o3/pro/video-to-video/edit';
...
const defaultFalAudioExtendModel = 'fal-ai/stable-audio-25/inpaint';
...
function App() {
  ...
  const [falVideoRestyleModel, setFalVideoRestyleModel] = useState(defaultFalVideoRestyleModel);
  const [falVideoReframeModels, setFalVideoReframeModels] = useState<main.FalModel[]>([]);
  const [falVideoRestyleModels, setFalVideoRestyleModels] = useState<main.FalModel[]>([]);
  ...
  const [falAudioExtendModel, setFalAudioExtendModel] = useState(defaultFalAudioExtendModel);
  ...
  const [replicateVideoUpscaleModel, setReplicateVideoUpscaleModel] = useState(defaultReplicateVideoUpscaleModel);
  const [replicateVideoUpscaleModels, setReplicateVideoUpscaleModels] = useState<main.ReplicateModel[]>([]);
  const [replicateVideoRestyleModel, setReplicateVideoRestyleModel] = useState(defaultReplicateVideoRestyleModel);
      ...
      SaveConfig(main.AppConfig.createFrom({
        ...
        providers: {
          ...
          fal: {
            ...
            videoRestyleModel: falVideoRestyleModel,
  ...
  const conversationModelSelection = useMemo<ModelSelectionValue>(() => {
    ...
    const next: ModelSelectionValue = {
      ...
      falVideoRestyleModel: overrides.videoRestyleModel && videoProvider !== 'replicate' ? overrides.videoRestyleModel : global.falVideoRestyleModel,
      falVideoReframeModel: overrides.videoReframeModel && videoProvider !== 'replicate' ? overrides.videoReframeModel : global.falVideoReframeModel,
  ...
  const patchSettingsModelSelection = (patch: Partial<ModelSelectionValue>) => {
    ...
    if (patch.falVideoRestyleModel !== undefined) setFalVideoRestyleModel(patch.falVideoRestyleModel);
  ...
  const patchConversationModels = (patch: Partial<ModelSelectionValue>) => {
    ...
    setConvModelsKeys((prev) => {
      ...
      if (patch.falVideoRestyleModel !== undefined || patch.replicateVideoRestyleModel !== undefined) next.add('videoRestyleModel');
    ...
    setConvModelsDraft((prev) => {
      ...
      switch (key) {
        ...
        case 'videoUpscaleModel':
          ...
          next.replicateVideoUpscaleModel = global.replicateVideoUpscaleModel;
          break;
        case 'videoReframeModel':
          next.falVideoReframeModel = global.falVideoReframeModel;
          next.replicateVideoReframeModel = global.replicateVideoReframeModel;
          break;
        case 'videoRestyleModel':
          next.falVideoRestyleModel = global.falVideoRestyleModel;
          next.replicateVideoRestyleModel = global.replicateVideoRestyleModel;
          break;
  ...
  const falVideoRestyleModelOptions = useMemo(() => falModelOptionList(falVideoRestyleModels), [falVideoRestyleModels]);
  ...
  async function loadConfig() {
	...
	const nextFalVideoRestyleModel = config.providers?.fal?.videoRestyleModel || defaultFalVideoRestyleModel;
    ...
    setFalVideoRestyleModel(nextFalVideoRestyleModel);
  ...
  async function refreshFalModels() {
    ...
    try {
      setFalVideoRestyleModels(asArray(await ListFalVideoRestyleModels()));
    } catch {
      setFalVideoRestyleModels([]);
    }
    try {
    ...
    try {
      setFalAudioExtendModels(asArray(await ListFalAudioExtendModels()));
    } catch {
      setFalAudioExtendModels([]);
    }
    try {
  ...
  async function refreshReplicateModels() {
    ...
    try {
      setReplicateVideoUpscaleModels(asArray(await ListReplicateVideoUpscaleModels()));
    } catch {
      setReplicateVideoUpscaleModels([]);
    }
    try {
      setReplicateVideoRestyleModels(asArray(await ListReplicateVideoRestyleModels()));
    } catch {
      setReplicateVideoRestyleModels([]);
    }
    try {
...
function overrideKeysFromRecord(overrides: main.ConversationModelOverrides | null | undefined): Set<string> {
  ...
  if (overrides.videoRestyleModel) keys.add('videoRestyleModel');
...
function conversationOverridesPayload(draft: ModelSelectionValue, keys: ReadonlySet<string>): main.ConversationModelOverrides {
  ...
  if (keys.has('videoRestyleModel')) payload.videoRestyleModel = draft.videoProvider === 'replicate' ? draft.replicateVideoRestyleModel : draft.falVideoRestyleModel;
...
function ModelSelectionPanel({
  ...
  catalogs: {
    ...
    falVideoRestyleModels: main.FalModel[];
  ...
  const falVideoRestyleOptions = falModelOptionList(catalogs.falVideoRestyleModels);
      ...
      <section className="settings-section">
        ...
        <div className="settings-rows">
          ...
          <div className="two-column">
            <div className="field">
              <div className="field-label-row">
                {fieldLabel('replicate-video-upscale-model', 'Video-Upscale Model (Replicate)', 'videoUpscaleModel')}
                <InfoHint
                  label="Video upscaling on Replicate"
                  text="upscale_video's endpoint when Replicate is the video provider. Factor-based models take the requested multiplier; resolution-tier models (topazlabs/video-upscale) derive their 720p/1080p/4K target from the source clip's own frame size times the factor, snapped up so an upscale never lands below the request."
                />
              </div>
              <ModelCombobox
                id="replicate-video-upscale-model"
                ariaLabel="Replicate video-upscale model"
                placeholder={defaultReplicateVideoUpscaleModel}
                value={value.replicateVideoUpscaleModel}
                onChange={(next) => onChange({replicateVideoUpscaleModel: next})}
                options={replicateVideoUpscaleOptions}
                allowCustom
              />
              {!catalogs.replicateHasKey ? (
                <span className="hint">Add a Replicate API token {keyWhere} to upscale videos.</span>
          ...
          <div className="two-column">
            <div className="field">
              <div className="field-label-row">
                {fieldLabel('replicate-video-reframe-model', 'Video-Reframe Model (Replicate)', 'videoReframeModel')}
                <InfoHint
                  label="Video reframing on Replicate"
                  text="Converts an attached clip to a new aspect ratio by generating the missing canvas area instead of cropping — the reframe_video tool's endpoint when Replicate is the video provider. The default (luma/reframe-video) reframes up to 30s at 720p."
                />
              </div>
              <ModelCombobox
                id="replicate-video-reframe-model"
                ariaLabel="Replicate video-reframe model"
                placeholder={defaultReplicateVideoReframeModel}
                value={value.replicateVideoReframeModel}
                onChange={(next) => onChange({replicateVideoReframeModel: next})}
                options={replicateVideoReframeOptions}
                allowCustom
              />
            </div>
            ...
            <div className="field">
              <div className="field-label-row">
                {fieldLabel('replicate-video-restyle-model', 'Video-Restyle Model (Replicate)', 'videoRestyleModel')}
                <InfoHint
                  label="Video restyling on Replicate"
                  text="Re-renders an attached clip under a style prompt while keeping its motion — the restyle_video tool's endpoint when Replicate is the video provider. The default (kling-v3-omni-video) edits via natural-language instruction and accepts attached reference images; wan-2.7-videoedit and luma/modify-video are the alternatives. Input clips are capped by the model."
                />
              </div>
              <ModelCombobox
                id="replicate-video-restyle-model"
                ariaLabel="Replicate video-restyle model"
                placeholder={defaultReplicateVideoRestyleModel}
                value={value.replicateVideoRestyleModel}
                onChange={(next) => onChange({replicateVideoRestyleModel: next})}
                options={replicateVideoRestyleOptions}
                allowCustom
              />
            </div>
          </div>
